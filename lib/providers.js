...
function makeGrokProvider({ model } = {}) {
  ...
  return {
    name: 'grok-cli', model: model || process.env.GROK_MODEL || 'grok-default',
    ...
    async generate(prompt, { cwd, model: m, effort, sandbox, maxTurns, timeoutMs } = {}) {
      const args = [...grokArgs({ model: (process.env.GROK_MODEL || m) ? (m || this.model) : null, effort, sandbox, maxTurns }), '-p', prompt];
      ...
      return {
        ...
        durationMs, responseId: (j && (j.session_id || j.id)) || null,
      };
    },
  };
}
