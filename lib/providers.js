...
function makeCodexProvider({ model } = {}) {
  return {
    ...
    async generate(prompt, { cwd, model: m, toolPolicy } = {}) {
      const args = ['exec', '--json', '--skip-git-repo-check', '--sandbox', 'read-only', '--color', 'never', '--ephemeral'];
      if (process.env.CODEX_MODEL || m) args.push('--model', m || this.model);
      args.push(...leanFlags(toolPolicy));
      args.push(prompt);
      const { stdout, durationMs } = runCommand('codex', args, { cwd });
