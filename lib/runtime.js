...
export async function runWork(opts = {}) {
  ...
  async function runTask(task) {
    ...
    const callModel = (!synthesis && route && (opts.subModel || agentSub)) ? (opts.subModel || agentSub) : opts.model;
    ...
    emit('provider_call_started', { taskId: task.id, provider: provider.name, label: task.owner, role: synthesis ? 'synthesis' : 'sub', model: callModel || provider.model, maxTokens: callMaxTokens, toolPolicy });
    const r = await provider.generate(prompt, { cwd: workdir, model: callModel, cachePrefix: stablePrefix, system: stablePrefix, maxTokens: callMaxTokens, toolPolicy });
