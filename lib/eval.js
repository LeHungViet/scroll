...
function judgePrompt(output, rubric) {
  return [
    `Bạn là giám khảo nghiêm khắc (${JUDGE_SENTINEL}). Chấm OUTPUT theo từng tiêu chí trong RUBRIC, thang 1–5.`,
    'CHỈ trả JSON, không giải thích: {"scores": {"<tiêu chí>": <1-5>}, "average": <số>}',
    '', '## RUBRIC', rubric || '(không có)', '', '## OUTPUT', output || '(rỗng)',
  ].join('\n');
}
async function judgeOutput(output, rubric, prov, model) {
  let res; try { res = await prov.generate(judgePrompt(output, rubric), { model }); }
...
export async function runEvals(name, { cwd = process.cwd(), n, provider = 'auto', model, judge = false, judgeProvider, judgeModel, onEvent } = {}) {
  ...
  for (const file of agent.evals) {
    ...
    if (judgeProv && gc.rubric) {
      judgeRes = await judgeOutput(lastText, gc.rubric, judgeProv, judgeModel || model);
