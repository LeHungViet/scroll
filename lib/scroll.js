  ...
  switch (cmd) {
    ...
    case 'eval': {
      ...
      for (const cs of rep.cases) {
        if (cs.skipped) { console.log(`${c.yellow('skip')} ${cs.id} — ${cs.reason}`); continue; }
        const mark = cs.verdict ? c.green('✔') : c.red('✖');
        const jd = cs.judge ? ` · judge ${cs.judge.ok ? cs.judge.average + '/' + cs.judge.min + (cs.judge.pass ? '✔' : '✖') : 'ERR ' + (cs.judge.error || '')}` : '';
        console.log(`${mark} ${c.bold(cs.id)} — ${cs.passCount}/${cs.runs} runs pass (consistency ${cs.consistency} ≥ ${cs.threshold}) · ${cs.checks} check(s)${jd}`);
        if (!cs.verdict) { const f = cs.perRun.find((r) => !r.allPass); if (f && f.fails.length) console.log(c.dim(`    failed: ${f.fails.join(', ')}${f.err ? ' · ' + f.err : ''}`)); }
      }
      const verdict = rep.ok ? c.green('PASS') : c.red('FAIL');
      console.log(`\n${verdict} — ${rep.passed}/${rep.gradable} gradable case(s) · provider ${rep.provider} (${rep.model}) · record .scroll/eval/${name}.json`);
